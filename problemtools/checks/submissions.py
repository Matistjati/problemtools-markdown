...
from ..model import DEFAULT_VALIDATOR, Graders, LegacyPolicy, Submission, Submissions, TestCase, TestDataGroup
from ..results_table import SubtaskResultsTable, get_table_groups
...
def check_submissions(
    ...
    table_groups = get_table_groups(testdata)
    table_ctx: SubtaskResultsTable | contextlib.AbstractContextManager[None] = (
        SubtaskResultsTable(
            table_groups,
            metadata,
            uses_default_validator=output_validator is DEFAULT_VALIDATOR,
            problem_name=probdir.name,
            show_subtask_scores=context.show_subtask_scores,
        )
        if table_groups
        else contextlib.nullcontext()
    )

    saved_status_callback = context.status_callback
    try:
        with table_ctx as results_table:
            ...
            # When the live table is up it already shows every result, per subtask; don't repeat it as text.
            if all_submission_results and results_table is None:
                _print_results_table(all_submission_results, testdata, metadata.is_scoring(), diag)
    finally:
        context.status_callback = saved_status_callback
...
def _check_submission_group(
    ...
    has_testcases: bool,
    diag: Diagnostics,
    results_table: SubtaskResultsTable | None = None,
...
def _table_category(sub: Submission, policy: LegacyPolicy) -> str:
    """Which verdict group the submission's row belongs to in the live results table."""
    if sub.directory == 'partially_accepted':
        return 'PAC'
    return policy.expected_verdict(sub) or 'AC'
...
def _check_submission(
    ...
    if partial and _fully_accepted(result, testdata, metadata):
        ...
        # The results table already shows this, per subtask; don't repeat it as a line of text.
    ...
    if results_table is not None:
        results_table.add_row(sub, results, label)
...
def _print_results_table(
    ...
    groups = get_table_groups(testdata)
